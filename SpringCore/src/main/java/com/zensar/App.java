...
public class App {
	...
	/*
	 * String[] str = iocContainer.getBeanDefinitionNames(); for(int i
	 * =0;i<str.length;i++) { if(str[i].contains("OrderBean")) { Order order =
	 * (Order)iocContainer.getBean(str[i]); System.out.println("The Order of Item "
	 * + i+1 + order); totalPrice += order.getItem().getPrice(); } }
	 ...
	 * for(int i=0;i<srt.length;i++) { Order order =
	 * (Order)iocContainer.getBean(srt[i]); System.out.println("The Order of Item "
	 * + i+1 + order); totalPrice += order.getItem().getPrice();
	 * 
	 * }
	 * 
	 * System.out.print("Total Price of All Orders : " + totalPrice);\
	 */
